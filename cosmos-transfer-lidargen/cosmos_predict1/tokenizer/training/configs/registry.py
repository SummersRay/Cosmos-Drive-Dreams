...
from cosmos_predict1.tokenizer.training.configs.base.callback import BASIC_CALLBACKS, FLOAT32_CALLBACKS, WANDBLIDAR_CALLBACK
...
from cosmos_predict1.tokenizer.training.configs.base.loss import VideoLossConfig, ImageLossConfig
...
from cosmos_predict1.tokenizer.training.datasets.lidar_datasets.tokenizer_dataloader import get_lidar_range_map_dataloader
...
def register_loss(cs):
    ...
    cs.store(group="loss", package="model.config.loss", name="image", node=ImageLossConfig)
...
def register_callback(cs):
    ...
    cs.store(group="callbacks", package="trainer.callbacks", name="float32", node=FLOAT32_CALLBACKS)
    cs.store(group="callbacks", package="trainer.callbacks", name="wandbLidar", node=WANDBLIDAR_CALLBACK)
